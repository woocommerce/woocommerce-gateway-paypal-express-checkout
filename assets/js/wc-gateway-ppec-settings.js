;(function ( $ ) {
	...
	$( function() {
		...
		// Legacy and checkout.js modes handling.
		var legacySettings = {
			init: function() {
				this._toggle_fields = '#woocommerce_ppec_paypal_cart_checkout_enabled,' +
									  '#woocommerce_ppec_paypal_mini_cart_settings_toggle,' +
				...
				// Update current mode based on environment and credentials.
				$( '#woocommerce_ppec_paypal_environment' ).change( $.proxy( this.maybeChangeMode, this ) );
				$( '#woocommerce_ppec_paypal_sandbox_api_client_id, #woocommerce_ppec_paypal_sandbox_api_secret, #woocommerce_ppec_paypal_live_api_client_id, #woocommerce_ppec_paypal_live_api_secret' )
					.on( 'keyup change', $.proxy( this.maybeChangeMode, this ) );

				// Update UI on changes associated to legacy code.
				$( this._toggle_fields ).change( $.proxy( this.showOrHideSettings, this ) );
				$( '.woocommerce_ppec_paypal_button_layout' ).change( $.proxy( this.showOrHideSettings, this ) );

				this.maybeChangeMode();
				this.showOrHideSettings();
			},

			maybeChangeMode: function() {
				var currentValue = ( 'undefined' !== typeof this.inLegacyMode ) ? this.inLegacyMode : undefined;

				this.environment  = $( '#woocommerce_ppec_paypal_environment' ).val();
				this.inLegacyMode = ! this.restCredentialsAvailable();

				if ( ( 'undefined' != typeof currentValue ) && ( currentValue != this.inLegacyMode ) ) {
					this.refreshUI();
				}
			},

			refreshUI: function() {
				// In legacy mode, only 'credit', 'card', 'venmo' and 'elv' are available as funding methods.
				$( ':input.woocommerce_ppec_funding_methods_select' )
					.find( 'option' )
					.not( 'option[value="credit"], option[value="card"], option[value="venmo"], option[value="elv"]' )
					.prop( 'disabled', this.inLegacyMode );
				$( ':input.woocommerce_ppec_funding_methods_select' ).removeClass( 'enhanced' );
				$( document.body ).trigger( 'wc-enhanced-select-init' );

				if ( this.inLegacyMode ) {
					$( this._toggle_fields ).change();
				} else {
					this.showOrHideSettings();
				}
			},
			...
			showOrHideSettings: function() {
				...
				$( '.woocommerce_ppec_legacy_setting' ).closest( 'tr' ).hide();

				// Always show the main "Hide Funding Method(s)" select and "PayPal Credit" checkbox when not in legacy mode.
				$( '#woocommerce_ppec_paypal_credit_enabled, #woocommerce_ppec_paypal_hide_funding_methods' ).closest( 'tr' ).show();
			},

			restCredentialsAvailable: function() {
				var fields      = [ '#woocommerce_ppec_paypal_' + this.environment + '_api_client_id', '#woocommerce_ppec_paypal_' + this.environment + '_api_secret' ];
				return fields.reduce( function( result, fieldId ) {
					var $field = $( fieldId );
					return result && ( 1 === $field.length && '' !== $field.val().trim() );
				}, true );
			},
