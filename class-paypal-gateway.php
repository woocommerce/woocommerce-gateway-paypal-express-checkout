...
function woo_pp_plugins_loaded() {
	class PayPal_Express_Checkout_Gateway extends WC_Payment_Gateway {
		...
		public function admin_options() {
			...
			if ( 'true' == $_GET['ips-signup'] ) {
				$this->ips_signup();
				WC_Admin_Settings::show_messages();
			}

			if ( 'true' == $_GET['ips-return'] ) {
				$this->ips_return();
			}
			...
			add_thickbox();
			require 'content/admin/settings.php';
		}
		...
		public function process_admin_options() {
			...
			if ( 'true' == $_POST['woo_pp_enabled'] ) {
				$enabled = true;
			}

			if ( 'true' == $_POST['woo_pp_ppc_enabled'] ) {
				$ppc_enabled = true;
			}

			if ( 'true' == $_POST['woo_pp_allow_guest_checkout'] ) {
				$allow_guest_checkout = true;
			}

			if ( 'true' == $_POST['woo_pp_block_echecks'] ) {
				$block_echecks = true;
			}

			if ( 'true' == $_POST['woo_pp_req_billing_address'] ) {
				$require_billing_address = true;
			}
			
			if ( 'true' == $_POST['woo_pp_icc_enabled'] ) {
				$icc_enabled = true;
			}
			...
			// WooCommerce_PayPal_Settings already has sanitizers for these values, so we don't need to check them.
			$button_size                = $_POST['woo_pp_button_size'];
			$mark_size                  = $_POST['woo_pp_mark_size'];
			$payment_action             = $_POST['woo_pp_payment_action'];
			$zero_subtotal_behavior     = $_POST['woo_pp_zero_subtotal_behavior'];
